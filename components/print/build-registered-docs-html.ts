...
export const buildRegisteredDocsHTML = (
  ...
  getDocTotals?: (id: string) => {
    goodsIn?: string
    goodsOut?: string
    goodsReceivable?: string
    goodsPayable?: string
    moneyIn?: string
    moneyOut?: string
  ...
  const rows = transactions.map((t, index) => {
    ...
    const isGoodsIn = t.type === "product_in"
    const isGoodsOut = t.type === "product_out"
  ...
  return `
    ...
    <body>
      <div class="header">
        <h1>${companyInfo?.nameFa || title}</h1>
        <p>${dateStr}</p>
        ${companyInfo?.phones?.length ? `<p>${companyInfo.phones.join(" - ")}</p>` : ""}
      </div>
